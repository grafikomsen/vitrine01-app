            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card card-secondary rounded-1">
                        ...
                        <form method="POST" name="editParternForm" id="editParternForm">
                            <div class="card-body">

                                <div class="form-group mb-3">
                                    <label for="name">Nom du blog</label>
                                    <input type="text" name="name" id="name" value="{{ $partern->name }}" class="form-control rounded-1" placeholder="Enter le nom du blog">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="description" id="description" rows="6" class="form-control rounded-1">{{ $partern->description }}</textarea>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-3">
                                            ...
                                            <textarea name="short_desc" id="short_desc" class="form-control rounded-1" cols="30" rows="7">{{ $partern->short_desc }}</textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="status" id="status" class="form-control rounded-1">
                                        <option {{ ($partern->status == 1) ? 'selected' : '' }} value="1">Activé</option>
                                        <option {{ ($partern->status == 0) ? 'selected' : '' }} value="0">Désactivé</option>
                                    </select>
                                </div>
                            </div>
...
@section('extraJs')
    ...
    <script>
        ...
        $('#editParternForm').submit(function(event){
            event.preventDefault();
            $("button[type='submit']").prop('disabled',true);
            $.ajax({
                url: '{{ route("admin.partners.updated",$partern->id) }}',
                type: 'PUT',
                dataType: 'json',
                data: $('#editParternForm').serializeArray(),
